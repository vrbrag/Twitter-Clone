...
######## TEST USER LIKES ########
   def setup_likes(self):
      ...
      # Set testuser to like message m3 (id=9876)
   ...
   def test_user_show_with_likes(self):
      ...
      with self.client as c:
         # Test user profile dashboard contain 4 stats: messages, followers, following, & likes
         resp = c.get(f"/users/{self.testuser_id}")
   ...
   def test_add_like(self):
      ...
      with self.client as c:
         # Need to be logged in as a current user to have authorization to 'like' any messages
         ...
         # find liked message, message_id=1984
         likes = Likes.query.filter(Likes.message_id==1984).all()
         self.assertEqual(len(likes), 1) 
         self.assertEqual(likes[0].user_id, self.testuser_id) # user id of liked message should == testuser's id
